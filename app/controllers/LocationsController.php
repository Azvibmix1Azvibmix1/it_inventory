...
class LocationsController extends Controller
{
  ...
  public function add()
  {
    ...
    if ($parentId) {
      ...
      $role = function_exists('currentRole') ? currentRole() : '';
      if (!in_array($role, ['super_admin', 'superadmin', 'manager'], true)) {
  ...
  public function edit($id = null)
  {
    ...
    // ✅ قطع الغيار بالموقع
    $spareSummary = null;
    if (method_exists($this->spareModel, 'getSpareStockSummary')) {
      $spareSummary = $this->spareModel->getSpareStockSummary($id);
    } elseif (method_exists($this->locationModel, 'getSpareStockSummary')) {
      $spareSummary = $this->locationModel->getSpareStockSummary($id);
    }

    $spareStocks = [];
    if (method_exists($this->spareModel, 'getSpareStocksByLocation')) {
      $spareStocks = $this->spareModel->getSpareStocksByLocation($id);
    } elseif (method_exists($this->locationModel, 'getSpareStocksByLocation')) {
      $spareStocks = $this->locationModel->getSpareStocksByLocation($id);
    }

    // ✅ سجل الحركات (مين صرف/ورد)
    $movements = method_exists($this->spareModel, 'getMovementsByLocation')
      ? $this->spareModel->getMovementsByLocation($id, 20)
      : [];
